...
class ApiClient {
  static Future getData(String url, {Map<String, String>? headers}) async {
    ...
    if (headers != null) Log.console('Http.Get Headers: $headers');

    try {
      final ioc = HttpClient()..badCertificateCallback = (X509Certificate cert, String host, int port) => true;
      final httpClient = IOClient(ioc);

      final response = await httpClient.get(Uri.parse(url), headers: headers).timeout(const Duration(seconds: 15));
